        ...
        <div class="row">
            <div class="col-md-8">
                ...
                <div class="post-container">
                    <?php 
                    $name_sql = "SELECT * FROM `category` WHERE `category_id` = $cat_id";
                    $name_run = mysqli_query($conn,$name_sql);
                    $namefetch = mysqli_fetch_assoc($name_run);
                    
                    ?>
                  <h2 class="page-heading"><?php echo $namefetch['category_name'] ?></h2>
                  <?php 
                   if (isset($_GET['cid'])) {
                    $cat_id =$_GET['cid'];
                }
                            $front_sql = "SELECT * FROM `post` p INNER JOIN category c ON p.category= c.category_id WHERE p.category = $cat_id ";
